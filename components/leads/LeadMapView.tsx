...
export function LeadMapView({
  ...
  const createSelectedLeads = async () => {
    if (selectedPlaces.size === 0) {
      Alert.alert(
        'No Selection',
        'Please select at least one business to add as leads.'
      );
      return;
    }
    ...
    try {
      ...
      for (const place of selectedPlaceObjects) {
        try {
          ...
          console.error(`Error creating lead for ${place.name}:`, error);
          errorCount++;
        }
      }
      ...
      if (successCount > 0) {
        Alert.alert(
          ...
          `Successfully created ${successCount} lead${
            successCount > 1 ? 's' : ''
          }${errorCount > 0 ? ` (${errorCount} failed)` : ''}`
        );
      } else {
        Alert.alert('Error', 'Failed to create any leads. Please try again.');
      }
      ...
      Alert.alert('Error', 'Failed to create leads. Please try again.');
    } finally {
                  ...
                  <>
                    ...
                    <TouchableOpacity
                      style={[
                        ...
                        isCreatingLeads && styles.disabledButton,
                      ]}
                      onPress={createSelectedLeads}
